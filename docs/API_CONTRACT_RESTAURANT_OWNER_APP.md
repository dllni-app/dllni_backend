...
- **Header (restaurant name + branch):**
  - `GET /api/v1/restaurants/{id}` – restaurant info (name, branch, logo).  
    - The owner app must know `restaurantId` from login context (out of scope of this file).
...
- **“طلبات جديدة” (new orders list) with Accept / Reject buttons:**
  - `GET /api/v1/orders?filter[restaurantId]={id}&filter[status]=pending` – list pending orders for this restaurant (admin contract §3.5).
...
- **“قيد التحضير” / “نشاط الطلبات (ساعات)” chart:**
  - `GET /api/v1/orders?filter[restaurantId]={id}&filter[createdToday]=1` – totals for today (per status).
  - `GET /api/v1/restaurant/dashboard/overview?restaurantId={id}` – `orderActivityByHour` used for the bar chart (admin dashboard contract §3.1).
...
- **Orders list with status tabs (e.g. الكل، قيد التحضير، جاهز، مكتمل، ملغي):**
  - `GET /api/v1/orders?filter[restaurantId]={id}&filter[status]={status}&perPage=&page=`.  
...
- **Products list with search, category filters, availability toggle:**
  - `GET /api/v1/products?filter[restaurantId]={id}&filter[search]=...&filter[isAvailable]=...` – base listing (admin contract §3.4).
  - `GET /api/v1/restaurant/search/products?filter[search]=...&filter[restaurantId]={id}` – full-text search for the search bar (admin contract §3.14).
...
- **“Products – After Add Menu” screen:**
  - Same `GET /api/v1/products?filter[restaurantId]={id}` listing showing newly created products.
...
- **Inventory home (المخزون) – cards and list:**
  - Summary cards (total items, low stock count, total value):  
    - `GET /api/v1/restaurant/inventory-summary?restaurantId={id}` (admin contract §3.7b).
  - Low-stock alert badge / list section:  
    - `GET /api/v1/restaurant/inventory-alerts?restaurantId={id}`.
  - Main inventory list + search:  
    - `GET /api/v1/inventory-items?filter[restaurantId]={id}&filter[search]=...` (admin contract §3.7a).
- **Inventory – Add new item (basic info + link to products):**
  - `POST /api/v1/inventory-items` – body includes `restaurantId`, `name`, `unit`, `quantity`, `minimumLimit`, `unitCost`, `productIds[]`.
  - `PUT/PATCH /api/v1/inventory-items/{id}` – update item and its linked `productIds[]`.
  - Product picker inside this form uses `GET /api/v1/products?filter[restaurantId]={id}`.

### 3.6 “More” screens (store info, hours, offers, coupons, staff, support)

- **Store information (معلومات المتجر):**
  - `GET /api/v1/restaurants/{id}` – load all store fields (logo URLs, cover, description, address, contact numbers, social links).
  - `PUT /api/v1/restaurants/{id}` – update basic info, address/location, and contact data (admin contract §3.2).
- **Working hours (ساعات العمل):**
  - `GET /api/v1/restaurants/{id}/operating-hours` – load daily schedule (admin contract §3.2a).
  - `PUT /api/v1/restaurants/{id}/operating-hours` – save weekly schedule.
