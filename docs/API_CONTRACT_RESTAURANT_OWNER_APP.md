...
- **JSON:** camelCase for all keys (e.g. `createdAt`, `perPage`, `permissionIds`).
...
- **Today’s balance / “نظرة عامة اليوم” card (total sales, orders today, etc.):**
  - `GET /api/v1/restaurant-owner/dashboard/performance` – see §4.1.  
    - Use `summary.totalOrders`, `summary.newOrdersCount`, `summary.confirmedOrdersCount`, `summary.completedOrdersCount`, `summary.totalRevenue`, `summary.averageOrderValue`, `summary.cancellationRatePercent`.
...
- **Employees & employee activity log:**
  ...
  - Permission picker for staff UI: `GET /api/v1/restaurant-owner/permissions` returns the direct permission catalog used when creating/updating employees (see section 8.6).
...
| summary.newOrdersCount              | number  | Count of new orders with status `pending` in the range. |
| summary.confirmedOrdersCount        | number  | Count of confirmed orders with status `accepted` in the range. |
| summary.completedOrdersCount        | number  | Count of completed orders with status `completed` in the range. |
...
- Selected permissions are synced directly onto the linked user.

**Response (201):** `{ "data": { ...employee resource... } }`

**Errors:**

- `422` � Validation (e.g. invalid email, invalid permission id).
...
| Method | Path                                      | Description |
| ------ | ----------------------------------------- | ----------- |
| GET    | `/api/v1/restaurant-owner/permissions`    | List direct permissions for employee assignment |
...
Use `permissions[].id` values when sending `permissionIds` in employee create/update requests.
...
Employees are now created as restaurant-linked users with direct permissions on the `users` model. Use `GET /api/v1/restaurant-owner/permissions` to render the permission picker and send the selected ids in `permissionIds`.
