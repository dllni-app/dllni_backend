...
- Name: owner-api.spec.ts >> Supermarket Owner API (Playwright request contexts) >> OWN-SM-06: owner can accept a pending order
- Location: tests\playwright\supermarket\owner-api.spec.ts:74:3
...
```ts
  ...
  3   | 
  4   | async function createPendingOrderViaUserFlow(
  5   |   roleRequests: {
  6   |     user: APIRequestContext;
  7   |   },
  8   |   productId: number,
  9   | ): Promise<number> {
