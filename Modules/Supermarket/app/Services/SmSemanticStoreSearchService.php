...
final class SmSemanticStoreSearchService
{
    /**
    ...
    {
        $baseUrl = (string) config('services.dallelni_search.stores_base_url');
        $authToken = (string) config('services.dallelni_search.auth_token');
        $timeout = (int) config('services.dallelni_search.timeout', 10);

        if ($baseUrl === '' || $authToken === '') {
            Log::warning('Semantic store search configuration is incomplete.');
        ...
        try {
            ...
            Log::warning('Semantic store search request failed.', [
