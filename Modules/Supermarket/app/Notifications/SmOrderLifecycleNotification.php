...
final class SmOrderLifecycleNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    private function payload(): array
    {
        ...
        return [
            ...
            'category' => 'orders',
            'priority' => $priority,
            'icon' => url('/images/notifications/supermarket.svg'),
            'title' => $title,
            'body' => $body,
            'message' => $body,
            'data' => $data,
    ...
    {
        $prefix = $this->targetRole === 'owner' ? 'supermarket.owner.order' : 'supermarket.order';
    ...
    private function copy(): array
    {
        ...
        $merchantName = (string) ($this->order->store?->name ?: 'المتجر');
        $customerName = (string) ($this->order->customer?->name ?: 'عميل');
        $statusLabel = $this->statusLabel($this->toStatus ?? $this->statusValue());

        if ($this->event === 'created' && $this->targetRole === 'owner') {
            return [
                'طلب جديد للمتجر',
                "وصل طلب سوبرماركت جديد رقم {$orderNumber} من {$customerName} ويحتاج متابعة.",
            ];
        }

        if ($this->event === 'created') {
            return [
                'تم إنشاء طلب السوبرماركت',
    ...
    private function data(): array
    {
        ...
        $deepLinkTarget = $this->targetRole === 'owner'
            ? 'supermarket_owner_order_details'
            : 'supermarket_order_details';
        ...
        return array_filter([
            ...
            'merchantId' => (int) $this->order->store_id,
            'merchantName' => (string) ($this->order->store?->name ?: ''),
            'status' => $status,
