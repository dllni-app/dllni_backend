...
final class UserCleaningOrderEstimationService
{
    ...
    public function pricingSnapshotInput(
        ...
        return [
            ...
            'propertyDetails' => $this->isEventAssistanceType($normalizedPropertyType)
    ...
    public function estimate(string $propertyType, array $propertyDetails, ?array $serviceIds = null): array
    {
        ...
        $calculation = $this->calculateRegularCleaningFromSettings(
            $this->normalizePropertyDetails($propertyDetails),
        );
        ...
        return [
            ...
            // Kept as a neutral response field for existing Flutter clients.
    ...
    private function calculateRegularCleaningFromSettings(array $normalizedDetails): array
    {
        ...
        return [
            ...
            'estimatedMinutesWithBuffer' => $rawMinutes,
            'setupBufferMinutes' => 0,
            'baseUnitPrice' => $baseUnitPrice,
            'deepCleaningMultiplier' => $deepMultiplier,
            'areaMarginMultiplier' => 1.0,
            'unitTotal' => round($unitTotal, 2),
            'modeMultiplier' => $modeMultiplier,
            'roomPricingLines' => $lines,
        ];
    }
    ...
    private function normalizeEventPropertyDetailsForStorage(array $propertyDetails): array
    {
        ...
        return array_filter([
            ...
            'location_name' => $this->nullableTrim($propertyDetails, 'location_name'),
    ...
    private function deriveLivingRoomSizeFromBreakdown(array $buckets): string
    {
        return $buckets['large'] > 0 ? 'large' : ($buckets['medium'] > 0 ? 'medium' : 'small');
    }

    private function sumRoomTypeBuckets(array $buckets): int
    {
        return $buckets['small'] + $buckets['medium'] + $buckets['large'];
    }

    private function sumAllRoomBuckets(array $breakdown): int
    {
        return array_sum(array_map(fn (array $buckets): int => $this->sumRoomTypeBuckets($buckets), $breakdown));
    }
    ...
    private function sizeTier(float $sqm): string
    {
        return $sqm < 80.0 ? 'small' : ($sqm < 140.0 ? 'medium' : ($sqm < 220.0 ? 'large' : 'very_large'));
    }

    private function normalizeCoordinate(mixed $coordinate): ?float
    {
        return is_numeric($coordinate) ? round((float) $coordinate, 6) : null;
    }

    private function normalizePreferredWorkerId(mixed $id): ?int
    {
        return is_numeric($id) && (int) $id > 0 ? (int) $id : null;
    }

    private function normalizeCleaningMode(mixed $mode): string
    {
        $mode = mb_strtolower(mb_trim((string) ($mode ?? 'regular')));

        return in_array($mode, self::CLEANING_MODES, true) ? $mode : 'regular';
    }
    ...
    private function normalizeRoomSize(string $size): string
    {
        ...
        return $size === 'very_large'
            ? 'large'
            : (in_array($size, CleaningFinancialDefaults::ROOM_SIZES, true) ? $size : 'medium');
    }
    ...
    private function normalizedRoomSizeRanges(mixed $value): array
    {
        ...
        foreach (self::ROOM_SIZE_BREAKDOWN_TYPES as $type) {
            foreach (CleaningFinancialDefaults::ROOM_SIZES as $size) {
                $min = $saved[$type][$size]['min'] ?? $defaults[$type][$size]['min'];
                $max = $saved[$type][$size]['max'] ?? $defaults[$type][$size]['max'];
                $average = $saved[$type][$size]['average'] ?? null;
                $defaults[$type][$size]['min'] = is_numeric($min) ? (float) $min : $defaults[$type][$size]['min'];
                $defaults[$type][$size]['max'] = is_numeric($max) ? (float) $max : $defaults[$type][$size]['max'];
                $defaults[$type][$size]['average'] = is_numeric($average)
                    ? (float) $average
                    : round(($defaults[$type][$size]['min'] + $defaults[$type][$size]['max']) / 2, 2);
            }
        }
    ...
    private function normalizedRoomTimeMinutes(mixed $value): array
    {
        ...
        foreach (self::ROOM_SIZE_BREAKDOWN_TYPES as $type) {
            foreach (CleaningFinancialDefaults::ROOM_SIZES as $size) {
                foreach (self::CLEANING_MODES as $mode) {
                    $savedValue = $saved[$type][$size][$mode] ?? null;
                    $defaults[$type][$size][$mode] = is_numeric($savedValue)
                        ? max(1, (int) $savedValue)
                        : $defaults[$type][$size][$mode];
                }
            }
        }
