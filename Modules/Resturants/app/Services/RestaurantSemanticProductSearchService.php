...
final class RestaurantSemanticProductSearchService
{
    /**
    ...
    {
        $baseUrl = (string) config('services.dallelni_search.restaurant_products_base_url', config('services.dallelni_search.products_base_url'));
        $authToken = (string) config('services.dallelni_search.auth_token');
        $timeout = (int) config('services.dallelni_search.timeout', 10);

        if ($baseUrl === '' || $authToken === '') {
            Log::warning('Semantic restaurant product search configuration is incomplete.');
        ...
        try {
            ...
            Log::warning('Semantic restaurant product search request failed.', [
