<x-filament-hub.page-shell>
    ...
    <x-filament::section :heading="__('cleaning_admin.financial.sections.worker_finance')">
        <div class="mb-4 rounded-xl border border-primary-200 bg-primary-50 p-4 text-sm text-primary-900 dark:border-primary-800 dark:bg-primary-950/30 dark:text-primary-100">الدين الإداري يضاف إلى رصيد الإيداع ويظهر بوضوح كدين يسترد أولاً عند إغلاق الحساب. المديونية مفهوم مستقل، وتنتج فقط عندما تتجاوز عمولات المنصة رصيد الإيداع، ويمكن للعامل الاستمرار ضمن حد المديونية المحدد.</div>
        <div class="grid gap-4 md:grid-cols-2">
            <div class="flex flex-col gap-1 md:col-span-2"><label class="flex items-center gap-2"><input type="checkbox" class="fi-checkbox rounded border-gray-300 dark:border-gray-600" wire:model.live="workerFinanceEnabled"><span class="text-sm">{{ __('cleaning_admin.financial.fields.worker_finance_enabled') }}</span></label><span class="text-xs text-gray-500 dark:text-gray-400">عند تعطيل هذا الخيار لن يتم تطبيق قيود السعة المالية على أهلية العامل لاستقبال طلبات جديدة.</span></div>
            <label class="flex flex-col gap-1"><span class="text-sm">حد المديونية الافتراضي</span><input type="number" min="0" step="0.01" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="defaultMaxNegativeBalance"><span class="text-xs text-gray-500 dark:text-gray-400">أقصى مديونية يسمح للعامل بالعمل ضمنها بعد نفاد رصيد الإيداع. يصبح الحساب المالي غير نشط عند تجاوز هذا الحد.</span>@error('defaultMaxNegativeBalance') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
