<x-filament-hub.page-shell>
    ...
    <x-filament::section :heading="__('cleaning_admin.financial.sections.worker_finance')">
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="flex flex-col gap-1"><span class="text-sm">عتبة التقييم المنخفض (نجوم)</span><input type="number" min="1" max="5" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="trustLowRatingThreshold"><span class="text-xs text-gray-500 dark:text-gray-400">يُخصم من نقاط الثقة إذا كان التقييم أقل من أو يساوي هذه العتبة.</span>@error('trustLowRatingThreshold') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
            <label class="flex flex-col gap-1"><span class="text-sm">خصم نقاط الثقة للتقييم المنخفض</span><input type="number" min="0" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="trustLowRatingPenalty">@error('trustLowRatingPenalty') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
            <label class="flex flex-col gap-1"><span class="text-sm">تفعيل إشعارات ما قبل المهمة</span><select class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="preTaskNotificationsEnabled"><option value="1">مفعّل</option><option value="0">معطّل</option></select>@error('preTaskNotificationsEnabled') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
            <label class="flex flex-col gap-1"><span class="text-sm">تذكير قبل المهمة (دقائق)</span><input type="number" min="10" max="180" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="preTaskReminderMinutes">@error('preTaskReminderMinutes') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
        </div>
    </x-filament::section>

    <x-filament::section heading="رسوم الإلغاء">
        <div class="grid gap-4 md:grid-cols-2">
            <label class="flex flex-col gap-1"><span class="text-sm">إلغاء مجاني للعميل حتى (ساعة)</span><input type="number" min="0" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="cancellationUserFreeUntilHours">@error('cancellationUserFreeUntilHours') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
            <label class="flex flex-col gap-1"><span class="text-sm">نسبة رسوم إلغاء العميل خلال 24 ساعة (%)</span><input type="number" min="0" max="100" step="0.01" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="cancellationUserWithin24hPercentage">@error('cancellationUserWithin24hPercentage') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
            <label class="flex flex-col gap-1"><span class="text-sm">نسبة رسوم إلغاء العميل خلال 12 ساعة (%)</span><input type="number" min="0" max="100" step="0.01" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="cancellationUserWithin12hPercentage">@error('cancellationUserWithin12hPercentage') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
            <label class="flex flex-col gap-1"><span class="text-sm">نسبة رسوم إلغاء العامل (%)</span><input type="number" min="0" max="100" step="0.01" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="cancellationWorkerFeePercentage"><span class="text-xs text-gray-500 dark:text-gray-400">تُحسب من إجمالي الحجز وتُسجَّل كمديونية على العامل عند الحاجة.</span>@error('cancellationWorkerFeePercentage') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
        </div>
    </x-filament::section>
