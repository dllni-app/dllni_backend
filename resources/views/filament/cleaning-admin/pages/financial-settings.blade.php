<x-filament-hub.page-shell>
    ...
    <x-filament::section :heading="__('cleaning_admin.financial.sections.worker_finance')">
        <div class="mb-4 rounded-xl border border-primary-200 bg-primary-50 p-4 text-sm text-primary-900 dark:border-primary-800 dark:bg-primary-950/30 dark:text-primary-100">يتم خصم عمولة المنصة من رصيد الإيداع أولاً. عند نفاد الإيداع فقط تتحول القيمة غير المغطاة إلى مديونية على العامل. لا يمكن أن يكون للعامل إيداع ومديونية موجبة في الوقت نفسه.</div>
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label class="flex flex-col gap-1"><span class="text-sm">حد المديونية الافتراضي المسموح</span><input type="number" min="0" step="0.01" class="fi-input block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800" wire:model.live="defaultMaxNegativeBalance"><span class="text-xs text-gray-500 dark:text-gray-400">أقصى مديونية يمكن أن يتحملها العامل بعد نفاد رصيد الإيداع. لا يستخدم النظام رصيداً سالباً بعد الآن.</span>@error('defaultMaxNegativeBalance') <span class="text-xs text-danger-600">{{ $message }}</span> @enderror</label>
