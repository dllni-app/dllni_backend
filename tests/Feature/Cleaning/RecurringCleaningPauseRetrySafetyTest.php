...
beforeEach(function (): void {
    ...
    CleaningFinancialSetting::query()->create(CleaningRuntimeSettings::financialDefaults());
});

it('treats repeated recurring pause requests as idempotent state assertions', function (): void {
    ...
    $this->postJson(
        ...
        ['reason' => 'توقف مؤقت بسبب السفر'],
    )
        ->assertOk()
        ->assertJsonPath('data.seriesAction.action', 'paused')
    ...
    $this->postJson(
        "/api/v1/cleaning-bookings/{$booking->id}/recurring/pause",
        ['reason' => 'إعادة إرسال بعد فقدان الاستجابة'],
    )
        ->assertOk()
        ->assertJsonPath('data.seriesAction.action', 'paused')
        ...
        ->assertJsonCount(0, 'data.seriesAction.releasedWorkerIds')
        ->assertJsonPath('data.schedule.isPaused', true)
        ->assertJsonPath('data.schedule.pauseReason', 'توقف مؤقت بسبب السفر');

    expect($booking->fresh()->recurring_paused_at?->toIso8601String())->toBe($pausedAt)
        ->and($booking->fresh()->recurring_pause_reason)->toBe('توقف مؤقت بسبب السفر')
        ->and((int) $firstSession->fresh()->version)->toBe($firstVersion)
...
it('treats repeated recurring resume requests as idempotent state assertions', function (): void {
    [$customer, $worker, $booking] = makeRecurringPauseRetryScenario();
    $firstSession = makeRecurringPauseRetrySession($booking, 1);
    $secondSession = makeRecurringPauseRetrySession($booking, 2);
    ...
    $this->postJson(
        ...
        ->assertOk()
        ->assertJsonPath('data.seriesAction.action', 'resumed')
    ...
    $this->postJson(
        ...
        ->assertOk()
        ->assertJsonPath('data.seriesAction.action', 'resumed')
        ->assertJsonCount(0, 'data.seriesAction.resumedSessionIds')
        ->assertJsonCount(0, 'data.seriesAction.expiredSessionIds')
        ->assertJsonPath('data.schedule.isPaused', false)
