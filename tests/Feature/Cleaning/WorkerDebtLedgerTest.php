...
it('records admin debt as indebtedness and consumes deposit first', function (): void {
    $worker = Worker::factory()->create(['trust_score' => 100, 'security_deposit_status' => 'active']);
    ...
    app(\Modules\Cleaning\Services\AdminCleaningTransactionService::class)->create(
        $worker,
        'debt',
        35000,
        'Manual indebtedness from admin panel',
        null,
    );

    $account = $worker->fresh('deposit')->deposit;
    expect((float) $account->current_balance)->toBe(0.0)
        ->and((float) $account->debt_balance)->toBe(15000.0);
});
