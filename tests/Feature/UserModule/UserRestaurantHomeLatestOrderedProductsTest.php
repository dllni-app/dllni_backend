...
it('returns distinct latest ordered products for the authenticated user', function (): void {
    ...
    $response->assertJsonPath('latestOrderedProducts.0.isFavorite', false);
...
it('sets isFavorite when latest ordered product is in authenticated user favorites', function (): void {
    $user = User::factory()->create();
    Sanctum::actingAs($user);
    ...
    $this->getJson('/api/v1/user/restaurants/home/latest-ordered-products')
        ->assertOk()
        ->assertJsonPath('latestOrderedProducts.0.productId', $product->id)
        ->assertJsonPath('latestOrderedProducts.0.isFavorite', true);
});
